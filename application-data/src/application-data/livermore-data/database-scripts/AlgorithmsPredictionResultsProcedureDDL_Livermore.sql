USE INTEL_SMART_ANTENNA_DATA_STORE_LIVERMORE;
DROP PROCEDURE IF EXISTS AlgorithmsPredictionResults;
DELIMITER $$
...
SET REF_TABLE_NAME = PARAM_REF_TABLE_NAME;
...
as VOTING_PREDICTED_Quadrant,PARAM_CORELATION_ID
 FROM REF_TABLE_NAME reference where reference.EPC 
	in (select distinct epc from CLOTHGROUPING_RESULT 
where CORELATION_ID=PARAM_CORELATION_ID);

END$$
DELIMITER ;
DROP PROCEDURE IF EXISTS AlgorithmsPredictionAccuracyResults;
DELIMITER $$
